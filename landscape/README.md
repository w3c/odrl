...
For example, dataspaces mention to use ODRL as usage control policy language for enforcement (both Gaia-X and IDSA). Within the [Solid](https://solidproject.org/TR/protocol) project, several people advocated to use ODRL for data usage control (see github issue [SAI-55](https://github.com/solid/authorization-panel/issues/55)).
...
| ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [ODRL Evaluator](https://w3id.org/force/evaluator)                                                   | An open implementation of an ODRL Evaluator that evaluates ODRL policies by generating Compliance Reports.<br> Written in Node.js and [Notation3](https://w3c-cg.github.io/N3/spec/) (prolog evaluated); runs in the [browser](https://w3id.org/force/demo) | [KNoWS](https://knows.idlab.ugent.be/) for the [Solidlab](https://solidlab.be/) project <br/> maintainer: [Wout Slabbinck](mailto:wout.slabbinck@ugent.be) |
...
| [ODRL Engine](https://github.com/DIPS-Tools/ODRL-Engine)                                             | An open implementation of an ODRL Evaluator supporting both Monitoring and Access Control scenarios, evaluating ODRL policies against a defined State of the World and/or access request, producing an evaluation report including allowed/not allowed results. Has [API](https://dips.soton.ac.uk/odrl-engine/api/docs) and [GUI](https://dips.soton.ac.uk/odrl-engine/odrl-engine-dashboard/) support. <br> Written in Python                                                                                      | University of Southampton                                                                                                                                  |
...
- [sovity EDC Community Edition (EDC CE)](https://github.com/sovity/edc-ce)
  - Extension of the [EDC](https://github.com/eclipse-edc/Connector)
- [O-prime](https://github.com/ahmadvoc12/Solid-Monitoring-Addons)
  - Framework for transparent monitoring of personal data usage, policy violation detection, and provenance tracking in decentralized [Solid](https://solidproject.org/TR/protocol) ecosystems.

### Protocols that point to ODRL

- [Dataspace Protocol (DSP)](https://eclipse-dataspace-protocol-base.github.io/DataspaceProtocol): The Dataspace Protocol is a specification designed to facilitate interoperable data sharing between entities governed by usage control and based on Web technologies. For usage control and contracts, the ODRL Vocabulary is reused.
- [Eclipse Dataspace Decentralized Claims Protocol (DCP)](https://eclipse-dataspace-dcp.github.io/decentralized-claims-protocol/): The DCP defines a set of protocols for asserting participant identities, issuing verifiable credentials, and presenting verifiable credentials using a decentralized architecture for verification and trust. 
  - Works orthogonally together with the DSP to solve the identity and trust gap.
- [Authorization for Data Spaces (A4DS)](https://spec.knows.idlab.ugent.be/A4DS/L1/latest/): A [User-Managed Access](https://docs.kantarainitiative.org/uma/wg/rec-oauth-uma-grant-2.0.html) extension that uses ODRL as the policy language for usage control.
...
- [ODRL Validation implementation](https://odrlapi.appspot.com/): User interface for validating ODRL policies against the Information Model.
- [Deontic Policy Consistency Checker](https://knowledgeonwebscale.github.io/n3s-policy-consistency-checker/): User interface to evaluate policies for internal inconsistencies. 
- [LLM4ODRL](https://github.com/Daham-Mustaf/LLM4ODRL): An ontology-guided approach for converting instructions into ODRL usage policies using Large Language Models. The repository also contains [SHACL](https://www.w3.org/TR/shacl/) shapes for ODRL.
- [ODRL2SHACL](https://github.com/paolo7/ODRL2SHACL)
- [ODRL Toolbox ](https://gitlab.com/gaia-x/lab/policy-reasoning/odrl-toolbox): A set of tools to validate, evaluate and check compliance for ODRL
- [bpmn2odrl](https://github.com/manabcodes/bpmn2odrl): Converts Business Process Model and Notation ([BPMN](https://www.bpmn.org/)) workflows into ODRL policies.
- [BAMBON](https://github.com/isotiropoulos/BayesianNegotiator): A learning-based negotiator that automatically negotiates ODRL based on the constraints imposed by the involved parties.
- [ODRL Atomization](https://github.com/woutslabbinck/ODRL-Atomization): Transforms compact ODRL policies with composite rules into policies with atomic rules.
- [ODRL Validator](https://github.com/elenamolino/odrl-validator): Validates ODRL policies using [SHACL](https://www.w3.org/TR/shacl/) and detects policy conflicts with [Notation3](https://w3c-cg.github.io/N3/spec/) reasoning.
- [Risk Detector Application (risk-e-tos)](https://github.com/isa-group/risk-e-tos): Explores ODRL policy risk assessment and mitigation suggestions using [Notation3](https://w3c-cg.github.io/N3/spec/) reasoning.
- [ODRL Engine](https://github.com/DIPS-Tools/ODRL-Engine): The ODRL Engine includes the following utility tools: a generator of synthetic ODRL policies, a generator of synthetic States of the World objects, and an ODRL validator. 
